...
__Rectangle::__Rectangle(__Graphics& Gfx,  D2D1_POINT_2F&& Middle, FLOAT Width, FLOAT Height,UINT Inside, UINT outside) :Inside(Inside), Outside(outside), Rect({ Middle.x - Width,Middle.y - Height, Middle.x + Width, Middle.y + Height }),Middle(Middle)
{
	...
	__Vector2D dx = { Width,0 }, dy = { 0,Height };
...
__Rectangle::__Rectangle(__Graphics& Gfx, const D2D1_POINT_2F& Middle, FLOAT Width, FLOAT Height, UINT Inside, UINT outside) :Inside(Inside), Outside(outside), Rect({ Middle.x - Width,Middle.y - Height, Middle.x + Width, Middle.y + Height }), Middle(Middle)
{
	...
	__Vector2D A = { 0,0 }, B = { 0,0 }, C = { 0,0 }, D = { 0,0 };
	__Vector2D dx = { Width,0 }, dy = { 0,Height };
	A = __Vector2D(Middle) - dx - dy;
	B = __Vector2D(Middle) + dx - dy;
	C = __Vector2D(Middle) - dx + dy;
	D = __Vector2D(Middle) + dx + dy;
	Vertices.emplace_back(std::move(A));
	Vertices.emplace_back(std::move(B));
	Vertices.emplace_back(std::move(D));
	Vertices.emplace_back(std::move(C));
...
void __Rectangle::Update( const Matrix::__Matrix3f& cBuffer, __Graphics& Gfx)
{
...
void __Rectangle::Draw(__Graphics& Gfx)
{
	HRESULT hr;
...
const auto __Rectangle::GetBuffer() const noexcept
{
	pBuffer->GetBuffer();
}
